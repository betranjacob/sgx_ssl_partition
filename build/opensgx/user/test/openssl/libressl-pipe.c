...
run_command_loop()
{
  ...
  if (sgxbridge_fetch_operation(&cmd_pkt, &data)) {
    ...
    fprintf(stderr, "Error fetching next operation\n");
  }
}
...
cmd_rsa_sign(cmd_pkt_t cmd_pkt, unsigned char* data)
{
  ...
  printf("\n Message Digest : len(%zu) ", cmd_pkt.data_len);

  if (RSA_sign(NID_md5_sha1, md_buf, cmd_pkt.data_len, signature, &sig_size,
...
void cmd_ecdhe_generate_pre_master_key(cmd_pkt_t cmd_pkt, unsigned char* data)
{
  ...
  if (EC_POINT_oct2point(group, clnt_ecpoint, data, cmd_pkt.data_len,
        bn_ctx) == 0) {
...
cmd_ssl_handshake_done(cmd_pkt_t cmd_pkt, unsigned char *data)
{
  ...
  if(memcmp(sgx_sess->s->session->session_id, zeros,
        SSL3_SSL_SESSION_ID_LENGTH) == 0){
...
cmd_sgx_tls1_enc(cmd_pkt_t cmd_pkt, unsigned char *data)
{
  ...
  if(sgx_tls1_enc->send){
    ...
    fprintf(stdout, "Opening input buffer (%zu)...\n",
        sgx_tls1_enc->len + sgx_tls1_enc->eivlen);
    print_hex(data + sizeof(sgx_tls1_enc_st),
        sgx_tls1_enc->len + sgx_tls1_enc->eivlen);
